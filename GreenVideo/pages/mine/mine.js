...
Page({
  ...
  uploadVideo: function () {
    ...
    wx.chooseVideo({
      sourceType: ['album'],
      success: function (res) {
        var duration = res.duration;//视频的时间长度
        var tmpheight = res.height;//视频的高度
        var tmpwidth = res.width;//视频的宽度
        var tmpVideoUrl = res.tempFilePath;//视频的临时文件路径 (本地路径
        var tmpCoverUrl = res.thumbTempFilePath;//临时图片(封面)文件路径
